...
func (f *flag) Init(cmd *cobra.Command) {
	cmd.Flags().StringVar(&f.Filename, flagFilename, "waves.svg", `Name of the output file.`)
	...
	cmd.Flags().StringVar(&f.Color1, flagColor1, "#4a69bd", `First color.`)
	cmd.Flags().StringVar(&f.Color2, flagColor2, "#34495e", `Second color.`)
	cmd.Flags().StringVar(&f.Color3, flagColor3, "#b8e994", `Third color.`)
