...
func (f *flag) Init(cmd *cobra.Command) {
	cmd.Flags().StringVar(&f.Filename, flagFilename, "octolines.svg", `Name of the output file.`)
	...
	cmd.Flags().StringVar(&f.Color1, flagColor1, "#1abc9c", `First color.`)
	cmd.Flags().StringVar(&f.Color2, flagColor2, "#b8e994", `Second color.`)
	cmd.Flags().StringVar(&f.Color3, flagColor3, "#ecf0f1", `Third color.`)
